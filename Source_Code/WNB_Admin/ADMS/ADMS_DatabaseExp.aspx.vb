...
Partial Public Class ADMS_DatabaseExp
...
#Region "--Events--"
    ...
    Private Sub btnUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Dim objBO As New ADMS_BAL_DatabaseExp

        Try


            intResult = objBO.UpdateDbExport(txtCode.Text.Trim, txtDesc.Text.Trim, Session("UserId"))
            ...
            If intResult = 1 Then
                ...
                "alert('DB Export Code details have been updated successfully.');", True)
            Else
                ScriptManager.RegisterStartupScript(Me, GetType(System.String), "Message", _
                "alert('Error occured while Updating DB Export Code details.');", True)
            End If
    ...
    Private Sub btnCreate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreate.Click
        ...
        Try
            intResult = objBO.CreateDbExport(txtCode.Text.Trim, txtDesc.Text.Trim, Session("UserId"))

            If intResult = 1 Then
                BindGridViewData()
                Clearcontrols()
                ScriptManager.RegisterStartupScript(Me, GetType(System.String), "Message", _
                "alert('DB Export Code has been saved successfully.');", True)

            ElseIf intResult = 0 Then
                ScriptManager.RegisterStartupScript(Me, GetType(System.String), "Message", _
                "alert('DB Export Code details already exist for " & txtCode.Text & " .');", True)
            Else
                ScriptManager.RegisterStartupScript(Me, GetType(System.String), "Message", _
                "alert('Error occured while Creating DB Export Code.');", True)
            End If
...
#Region "--Methos Or Functions--"
    ...
    Private Sub DeleteAirportMax(ByVal strSelectedCat As String)
        ...
        Try
            intResult = objBO.DeleteDbExport(strSelectedCat, Session("UserId"))
            ...
            If intResult = 1 Then
                ...
                Clearcontrols()
                ScriptManager.RegisterStartupScript(Me, GetType(System.String), "Message", _
               "alert('DB Export Code has been deleted successfully.');", True)
            Else
                ScriptManager.RegisterStartupScript(Me, GetType(System.String), "Message", _
                "alert('DB Export Code is in use, could not delete.');", True)
            End If

        Catch ex As Exception
            ScriptManager.RegisterStartupScript(Me, GetType(System.String), "Message", _
             "alert('Error occured while deleting DB Export Code.');", True)
        Finally
            objBO = Nothing
        End Try
    End Sub
    ...
    Private Function IsValidate(ByVal strCmd As String) As Boolean
        ...
        Catch ex As Exception
            ScriptManager.RegisterStartupScript(Me, GetType(System.String), "Message", "alert('Error occured while validating DB Export Code.');", True)
        End Try
    ...
    Private Sub BindGridViewData()
        ...
        Try
            dtBU = objBO.GetDbExport(Session("UserId"))
    ...
    End Sub
    ''' <summary>
    ''' To Update DB Export Code, page controls should be filled with selected AptMAxExp details
    ''' </summary>
    ''' <remarks></remarks>
