Public Partial Class GalleyArms
    ...
    Protected Sub ddlSublfeetID_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ddlSubfleetID.SelectedIndexChanged
        ...
        Try
            If (ddlSubfleetID.SelectedIndex <> -1) Then
                ...
                dtGallyArms = go_Bo.Get_GalleyArms(hidTableId.Value, ddlAircraftID.SelectedValue.ToString, "GALLEY_DESIG", ddlSubfleetID.SelectedValue.ToString)
                gvGalleyArms.DataSource = dtGallyArms
                gvGalleyArms.DataBind()
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            For intRCnt As Integer = 0 To gvGalleyArms.Rows.Count - 1

                GalleyDesignation = gvGalleyArms.Rows(intRCnt).Cells(0).Text

                strAircraftId = gvGalleyArms.Rows(intRCnt).Cells(0).Text
                If (CType(gvGalleyArms.Rows(intRCnt).Cells(2).FindControl("hidVersionNo"), HiddenField).Value <> "") Then
                    intVersionNo = CInt(CType(gvGalleyArms.Rows(intRCnt).Cells(2).FindControl("hidVersionNo"), HiddenField).Value)
                End If
                If (CType(gvGalleyArms.Rows(intRCnt).Cells(1).FindControl("TxtArm"), TextBox).Text <> "") Then
                    Arm = CDec(CType(gvGalleyArms.Rows(intRCnt).Cells(1).FindControl("TxtArm"), TextBox).Text)
                End If
                If (CType(gvGalleyArms.Rows(intRCnt).Cells(3).FindControl("hidCrewGalleyArmID"), HiddenField).Value <> "") Then
                    CrewGallyArmID = Convert.ToInt64(CType(gvGalleyArms.Rows(intRCnt).Cells(3).FindControl("hidCrewGalleyArmID"), HiddenField).Value)
                End If
                If (intVersionNo = 0) Then
                    intVersionNo = -1
                End If
                'CABIN_CREW_DESIGN
                intResult = go_Bo.Create_Update_GalleyArms(Convert.ToInt64(CrewGallyArmID), GalleyDesignation, "GALLEY_DESIG", Arm, ddlAircraftID.SelectedValue.ToString, intVersionNo, ddlSubfleetID.SelectedValue.ToString, "Admin")
