Partial Public Class UnderFloorPostionDetail
    ...
    Private Sub GetULDDetails()

        Dim dtULDDetails As DataTable
        Try
            dtULDDetails = go_Bo.Get_ULDPosition(hidTableId.Value, ddlAircraft.SelectedValue.Trim, 0)
    ...
    Protected Sub gvPosition_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles gvPosition.RowEditing
        ...
        Try
            ...
            PositionID = Convert.ToInt32(CType(gvPosition.Rows(e.NewEditIndex).Cells(4).FindControl("hidAirCraftPosID"), HiddenField).Value)
            hidVersion = Convert.ToInt32(CType(gvPosition.Rows(e.NewEditIndex).Cells(4).FindControl("hidVersion"), HiddenField).Value)
            Response.Redirect("UlDPositionEdit.aspx?FID=" & hidFunctionId.Value & "&TID=" & hidTableId.Value & "&AID=" _
                              & strAircraftId & "&PosID=" & PositionID & "&CID=" & ddlAircarftConfig.SelectedValue.Trim & _
    ...
    Protected Sub btnAdd_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAdd.Click
        ...
        Try
            If (ddlAircraft.SelectedIndex <> -1 And ddlAircarftConfig.SelectedIndex <> -1 And ddlULDConfiguration.SelectedIndex <> -1) Then
                Response.Redirect("UlDPositionEdit.aspx?FID=" & hidFunctionId.Value & "&TID=" & hidTableId.Value & "&AID=" & ddlAircraft.SelectedValue.Trim & "&PosID=" & "&CID=" & ddlAircarftConfig.SelectedValue.Trim & "&UID=" & ddlULDConfiguration.SelectedValue & "&VID=0")
            Else
                ScriptManager.RegisterStartupScript(Me, GetType(System.String), "Message", _
